    ...
    def Processingـincomingـmessages(self):
        if self.dict_messages != []:
            ...
            if not message['fromMe']:
                chatID  = message['from'] 
                if len(text) > 0 and text[0].lower() == 'hi':
                    return self.welcome(chatID)
                elif len(text) > 0 and text[0].lower() == 'time':
                    return self.time(chatID)
                elif len(text) > 0 and text[0].lower() == 'image':
                    return self.send_image(chatID)
                elif len(text) > 0 and text[0].lower() == 'video':
                    return self.send_video(chatID)
                elif len(text) > 0 and text[0].lower() == 'audio':
                    return self.send_audio(chatID)
                elif len(text) > 0 and text[0].lower() == 'voice':
                    return self.send_voice(chatID)
                elif len(text) > 0 and text[0].lower() == 'contact':
